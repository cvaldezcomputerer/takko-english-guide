 ...
 * image pipeline; reuses the review gallery (scripts/review-server.mjs).
 *
 * Requires PEXELS_API_KEY and/or PIXABAY_API_KEY in .env.
 *
 *   node scripts/find-image.mjs add "<query>" ["<query>" ...]   # fetch candidates
 *   node scripts/find-image.mjs serve [--port 4400]             # open the pick gallery
 *   node scripts/find-image.mjs list                            # show queue + picks
...
const DIR = join(__dirname, ".find-image");
...
async function cmdAdd() {
  ...
  console.log(`Now run:  node scripts/find-image.mjs serve   then open the gallery and pick.`);
}
