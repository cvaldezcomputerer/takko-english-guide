 ...
 * scripts/image-candidates.json (step 1) and writes your picks to
 * scripts/image-selections.json. Picks save instantly, so you can stop and
 * resume anytime.
 *
 * Usage:
 *   node scripts/review-server.mjs        then open http://localhost:4321
 *
...
const CANDIDATES_PATH = process.env.CANDIDATES_PATH || join(__dirname, "image-candidates.json");
const SELECTIONS_PATH = process.env.SELECTIONS_PATH || join(__dirname, "image-selections.json");
